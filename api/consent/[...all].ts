...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // GET /api/consent - list user consents
  if (req.method === 'GET' && !purpose) {
  ...
  if (req.method === 'PATCH' && purpose) {
    ...
    try {
      ...
      return res.status(200).json({
        ...
        data: { purpose, withdrawn: parsed.data.withdrawn, notice: impactStatement }
      });
    } catch (err: any) {
